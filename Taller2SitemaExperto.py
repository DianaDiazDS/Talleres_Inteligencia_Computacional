...
class SistemaExpertoIsquemia(KnowledgeEngine):
    ...
    @Rule(Paciente(SEXO='m', ED='joven', DM='no', AF='no', CO='alto', HF='si', HA='no', DR='no',
                              FEE='no', MT='no', EP='no', DEM='no', NV='no', SFM='no', DTM='no', DTA='no',
                              DTT='no'))
    def regla_13(self):
    ...
    @Rule(Paciente(SEXO='m', ED='joven', DM='no', AF='no', CO='bajo', HF='no', HA='no', IMC='grado1',
                              DR='no', FEE='no', MT='no', EP='no', DEM='no', NV='no', SFM='no', DTM='no',
                              DTA='no', DTT='no'))
    def regla_12(self):
        print("No tiene la enfermedad coronaria")
    ...
    @Rule(Paciente(SEXO='h', ED='adulto', DM='si', AF='no', CO='alto', HF='si', HA='si', DR='si',
                              FEE='si', MT='si', EP='si', DEM='si', NV='no', SFM='si', DTM='no', DTA='si',
                              DTT='no'))
    def regla_4(self):
        print("Tiene Angina pecho estable")

    @Rule(Paciente(SEXO='h', ED='adulto', DM='si', AF='no', CO='muy alto', HF='si', HA='si', DR='si',
                              FEE='si', MT='si', EP='si', DEM='si', NV='no', SFM='si', DTM='si', DTA='no',
                              DTT='no'))
    def regla_5(self):
    ...
    @Rule(Paciente(SEXO='h', ED='adulto', DM='no', AF='no', CO='bajo', HF='si', HA='si', DR='no',
                              FEE='no', MT='si', EP='si', DEM='si', NV='no', SFM='no', DTM='no', DTA='si',
                              DTT='no'))
    def regla_7(self):
        print("Tiene Angina pecho estable")

    @Rule(Paciente(SEXO='h', ED='mayor', DM='si', AF='si', CO='muy alto', HF='si', HA='si', DR='si',
                              FEE='si', MT='si', EP='si', DEM='si', NV='si', SFM='si', DTM='si', DTA='no',
                              DTT='no'))
    def regla_8(self):
        print("Tiene Angina pecho inestable")
    ...
    @Rule(Paciente(SEXO='h', ED='mayor', DM='no', AF='no', CO='muy alto', HF='si', HA='si', DR='no',
                              FEE='no', MT='si', EP='no', DEM='no', NV='no', SFM='no', DTM='no', DTA='no',
                              DTT='no'))
    def regla_9(self):
    ...
    @Rule(Paciente(SEXO='m', ED='adulta', DM='si', AF='no', CO='alto', HF='no', HA='si', DR='no',
                              FEE='no', MT='no', EP='si', DEM='no', NV='no', SFM='no', DTM='si', DTA='no',
                              DTT='no'))
    def regla_15(self):
...
def obtener_diagnostico_isquemia():
    ...
    evaluar_isquemia(sexo, edad, diabetes, antecedentes_familiares, colesterol, fumar, hipertension, dolor_toracico,
                     fatiga, edema, dolor_espalda, nauseas_vomitos, sudoracion, dolor_toracico_menos_un_mes,
...
entry_fatiga.pack()
label = tk.Label(root, text="edema")
label.pack()
entry_edema = tk.Entry(root)
entry_edema.pack()
label = tk.Label(root, text="espalda")
label.pack()
entry_dolor_espalda = tk.Entry(root)
entry_dolor_espalda.pack()
label = tk.Label(root, text="vomito")
label.pack()
entry_nauseas_vomitos = tk.Entry(root)
entry_nauseas_vomitos.pack()
entry_sudoracion = tk.Entry(root)
label = tk.Label(root, text="sudoracion")
label.pack()
entry_sudoracion.pack()
